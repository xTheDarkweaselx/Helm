...
public struct Styles: Codable, Equatable {
  ...
  // Helm fork (2026): do NOT decode <dxfs>. Differential (conditional-format)
  // entries have a different shape — their <dxf> Formats omit numFmtId/fontId/
  // etc. — so decoding them into the required-field `Format` throws on real
  // files. Helm never uses dxfs. (Removed: differentialFormats / "dxfs".)
