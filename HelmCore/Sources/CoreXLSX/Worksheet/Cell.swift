...
public enum CellType: String, Codable {
  ...
  // Modified by the Helm project (2026): tolerate unknown cell-type values
  // instead of aborting the whole worksheet parse. Callers treat `.unknown`
  // like a typeless cell and fall back to `Cell.value`.
  case unknown

  public init(from decoder: Decoder) throws {
    let raw = try decoder.singleValueContainer().decode(String.self)
    self = CellType(rawValue: raw) ?? .unknown
  }
}
