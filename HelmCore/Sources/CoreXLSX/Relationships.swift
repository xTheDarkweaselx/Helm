...
public struct Relationship: Codable, Equatable {
  public enum SchemaType: String, Codable {
    ...
    // HELM FORK PATCH (Spike 2): tolerate relationship types not in this allow-list
    // (e.g. Microsoft sensitivity-label "classificationlabels", "sheetMetadata")
    // instead of aborting the whole parse.
    case unknown = "__helm_unknown__"

    public init(from decoder: Decoder) throws {
      let raw = try decoder.singleValueContainer().decode(String.self)
      self = SchemaType(rawValue: raw) ?? .unknown
    }
  }
